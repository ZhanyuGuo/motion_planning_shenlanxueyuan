    ...
    for (int i = -1; i <= 1; ++i)
    {
        for (int j = -1; j <= 1; ++j)
        {
            for (int k = -1; k <= 1; ++k)
            {
                ...
                Vector3i offset(i, j, k);
                nIdx = currentIdx + offset;
                if ((nIdx(0) < 0) || (nIdx(1) < 0) || (nIdx(2) < 0) || (nIdx(0) > GLX_SIZE - 1) || (nIdx(1) > GLY_SIZE - 1) || (nIdx(2) > GLZ_SIZE - 1))
                {
                    continue;
                }
                ...
                nPtr = GridNodeMap[nIdx(0)][nIdx(1)][nIdx(2)];
                neighborPtrSets.push_back(nPtr);

                tmpGn = getCost(currentPtr, nPtr);
                edgeCostSets.push_back(tmpGn);
            }
        }
    }
}

double AstarPathFinder::getCost(GridNodePtr node1, GridNodePtr node2)
{
    double g;
    ...
    {
    case Euclidean:
        g = (node1->index - node2->index).cast<double>().norm();
        break;
    case Manhattan:
        g = (node1->index - node2->index).lpNorm<1>();
        break;
    default:
        g = (node1->index - node2->index).cast<double>().norm();
        break;
...
double AstarPathFinder::getHeu(GridNodePtr node1, GridNodePtr node2)
{
    ...
    case Diagonal:
    {
        Vector3i dr = (node1->index - node2->index);
        int dx = abs(dr(0));
        int dy = abs(dr(1));
        int dz = abs(dr(2));
        int dt = min(min(dx, dy), dz);
        if (dt == dx)
        {
            h = sqrt(3) * dt + sqrt(2) * min(dy - dt, dz - dt) + abs(dy - dz);
        }
        else if (dt == dy)
        {
            h = sqrt(3) * dt + sqrt(2) * min(dx - dt, dz - dt) + abs(dx - dz);
        }
        else
        {
            h = sqrt(3) * dt + sqrt(2) * min(dx - dt, dy - dt) + abs(dx - dy);
        }
        break;
    }
    default:
...
void AstarPathFinder::AstarGraphSearch(Vector3d start_pt, Vector3d end_pt)
{
    ...
    while (!openSet.empty())
    {
        ...
        for (int i = 0; i < (int)neighborPtrSets.size(); i++)
        {
             ...
             * please write your code below
             * id: 1--> open set, -1 --> closed set
             */
            neighborPtr = neighborPtrSets[i];
            double g = currentPtr->gScore + edgeCostSets[i];
...
vector<Vector3d> AstarPathFinder::getPath()
{
    ...
    /**
     * STEP 8:  trace back from the curretnt nodePtr to get all nodes along the path please write your code below
     */
